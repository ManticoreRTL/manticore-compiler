...
{inits_str}
    ...
    def generateTracker(name: str, signature) -> str:
        tracks = " " + \
            '\n '.join(
                [f'- "Tester_{name}/dut/{field.lower()}"' for field in signature])
        return f"""
wires:
{tracks}
...
"""
    ...
    def generate(name: str, signature: typing.List[str]) -> None:
        ...
        writeToFile(name + "_track.yml", generateTracker(name, signature))
