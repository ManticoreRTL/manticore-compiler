...
object WidthConversion {
  ...
  // multiple instructions and turns every name into multiple names. Sadly enough
  // the pass outputs code that is not SSA. Hence we need to run UnconstrainedRenameVariables
  // to make the code SSA. Furthermore, the core only lazily creates new registers when
  // it sees a register either at use or def positions. This means that if there
  // is an InputType register that is never read (i.e., consider a counter register
  // which the user only expects to read in the VCD dumps!) then this register
