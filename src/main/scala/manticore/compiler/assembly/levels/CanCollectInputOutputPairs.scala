...
/** Helper functional trait for building passes in which input and output pairs
  * need to be "closed". See [[manticore.assembly.levels.CloseSequentialCycles]]
  * for an example.
  * @author
  *   Mahyar Emami <mahyar.emami@eplf.ch>
  */
trait CanCollectInputOutputPairs extends Flavored {
      ...
      val ids_to_current_regs = user_regs
        .filter { r =>
          ...
          reg_annon match {
            case Some(reg) =>
              reg
                .get(AssemblyAnnotationFields.Type)
                .get == RegAnnotation.Current
            case _ =>
              false
          }
